        ...
        void SortIfNeeded(TreeViewItem root, IList<TreeViewItem> rows)
        {
            ...
            switch (sortedColumnIndex)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    items = items.Order(i => i.data.ForceToMono, ascending);
                    break;
            }
        ...
        private void CellGUI(Rect cellRect, TreeViewItem<AudioTreeItem> item, int column, ref RowGUIArgs args)
        {
            ...
            switch (column)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if (item.data.IsQualityOK())
                        GUI.Label(cellRect, item.data.Quality.ToString(), OptimizerWindow.okStyle);
                    else
                        GUI.Label(cellRect, item.data.Quality.ToString(), OptimizerWindow.warningStyle);
                    break;
                case 3:
                    if (item.data.IsMonoOK())
                        GUI.Label(cellRect, item.data.ForceToMono.ToString(), OptimizerWindow.okStyle);
                    else
                        GUI.Label(cellRect, item.data.ForceToMono.ToString(), OptimizerWindow.warningStyle);
                    break;
                case 4:
                    if (!item.data.IsQualityOK() || !item.data.IsMonoOK())
                    {

        ...
        void SortIfNeeded(TreeViewItem root, IList<TreeViewItem> rows)
        {
            ...
            switch (sortedColumnIndex)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    items = items.Order(i => i.data.TextureFormat, ascending);
                    break;
                case 4:
                    items = items.Order(i => i.data.TextureFormat, ascending);
                    break;
        ...
        private void CellGUI(Rect cellRect, TreeViewItem<TextureTreeItem> item, int column, ref RowGUIArgs args)
        {
            ...
            switch (column)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    if (item.data.IsFormatOK())
                        GUI.Label(cellRect, item.data.TextureFormat.ToString(), OptimizerWindow.okStyle);
                    else
                        GUI.Label(cellRect, item.data.TextureFormat.ToString(), OptimizerWindow.warningStyle);
                    break;
                case 4:
                    if (!item.data.IsFormatOK())
                    {
